function [Klocal, Flocal]=GeomExactBeam_2D(elmDat, IEN, e, XX, td, solnCur, veloCur, acceCur, bf)
...
uxn(1) = solnCur(ndof*(IEN(e,3)-1)+1);
uzn(1) = solnCur(ndof*(IEN(e,3)-1)+2);
btn(1) = solnCur(ndof*(IEN(e,3)-1)+3);

uxn(2) = solnCur(ndof*(IEN(e,4)-1)+1);
uzn(2) = solnCur(ndof*(IEN(e,4)-1)+2);
btn(2) = solnCur(ndof*(IEN(e,4)-1)+3);
...
nGP = 1;
[gp, gw] = get_Gauss_points(nGP);

    for mm = 1:nGP
        
        [N,dN_dx,d2N_dx2,J,xcoord]=shape_functions_Lagrange_1D(IEN(e,3:end), XX, p, gp(mm));
        Jmod = J*gw(mm);
...
Klocal = RotMat*Klocal;
Flocal = RotMat*Flocal;
Klocal = Klocal*RotMat';
...
res(1) = acceCur(ndof*(IEN(e,3)-1)+1);
res(2) = acceCur(ndof*(IEN(e,3)-1)+2);
res(3) = acceCur(ndof*(IEN(e,3)-1)+3);
res(4) = acceCur(ndof*(IEN(e,4)-1)+1);
res(5) = acceCur(ndof*(IEN(e,4)-1)+2);
res(6) = acceCur(ndof*(IEN(e,4)-1)+3);

Klocal = Klocal + d1*Mlocal;
Flocal = Flocal - Mlocal*res;
